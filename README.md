...
- The `src/index.js` also needs to be changed because the built version should load the JSON data from somewhere else (our express app API) and to do that we create a function:
  ...
  ```
- Below is a basic server which takes the data from the JSON files of the `src/static` directory
  - Save as `src/server/app.js`
  - **Read** its content **carefully**
  - Change the routes so that they use mysql
  
<details><summary>src/server/app.js</summary>
